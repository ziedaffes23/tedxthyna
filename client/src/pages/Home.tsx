...
export default function Home() {
  ...
  return <div className="reference-home-v2">
    ...
    <header className="reference-header"><a href="/" className="reference-brand"><img src={logo} alt="" /><span>TEDx <b>THYNA</b></span></a><nav className="reference-nav">{navigation.map(([label, href]) => <a href={href} key={href}>{label}</a>)}<a className="reference-register" href="/register">REGISTER <ArrowUpRight size={14} /></a></nav><button className="reference-menu" onClick={() => setMenu(!menu)} aria-label="Toggle navigation">{menu ? <X /> : <Menu />}</button></header>
