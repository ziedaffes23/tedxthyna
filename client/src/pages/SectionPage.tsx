...
function Board() {
  const clues = [["TEDx THYNA", "ORIGIN"], ["THE MINDS", "LINKED"], ["IDEAS", "LINKED"], ["THE SCENE", "LINKED"], ["THE MISSION", "PURPOSE"]];
  return <><PageIntro number="03" label="INVESTIGATION BOARD" title={<>Everything<br /><em>connects.</em></>} lead="The evidence map links the mission, the minds, the scene and every idea in one active case file." /><main className="route-body section-pad board-route"><div className="board-frame" style={{ backgroundImage: `linear-gradient(90deg, rgba(5,5,5,.78), rgba(5,5,5,.26)), url(${boardImage})` }}><div className="board-head"><span className="mono red">EVIDENCE MAP / LIVE</span><h2>Everything<br /><em>connects.</em></h2></div><div className="clue-map">{clues.map(([clue, status], i) => <div key={clue} className={`clue clue-${i}`}><span>{String(i + 1).padStart(2, "0")}</span><b>{clue}</b><small>{status}</small></div>)}</div><div className="board-thread thread-one" /><div className="board-thread thread-two" /><div className="board-thread thread-three" /><div className="board-caption mono">SELECT A CLUE TO CONTINUE THE INVESTIGATION</div></div></main></>;
}
